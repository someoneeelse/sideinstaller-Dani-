...
final class SideBySideManager: ObservableObject {
    ...
    /// PIN generated by the iOS 27 Remote Pairing fallback for the target iPhone.
    ...
    private func connectToTarget(ip: String) async throws {
        ...
        do {
            ...
            remotePairingPIN = nil
            // PairingController normally exposes its PIN on the Install tab.
            // Side by Side has its own screen, so mirror that PIN here while
            // the target is completing Remote Pairing.
            let pinMirror = Task { @MainActor [weak self] in
                while !Task.isCancelled {
                    if let pin = Engine.shared.pairingPIN, !pin.isEmpty {
                        self?.remotePairingPIN = pin
                    }
                    try? await Task.sleep(for: .milliseconds(100))
                }
            }
            defer { pinMirror.cancel() }
...
struct SideBySideView: View {
    ...
    private func detail(for step: SideBySideStep) -> String? {
        ...
        case .connect:
            if let pin = manager.remotePairingPIN {
                return L("Enter code %@ on their iPhone…", pin)
            }
            return L("Waiting for pairing approval…")
        case .download:
